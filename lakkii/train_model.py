...
if __name__ == "__main__":
    ...
    with common.RewardTracker(writer, np.inf, group_rewards=100) as reward_tracker:
        while True:
            ...
            # replay bufferに追加
            buffer.populate(1)
            # ε-greedyで1Mまでを計算してεを決定。　EPSIRON_STEPS : 1M
            selector.epsilon = max(EPSILON_STOP, EPSILON_START - step_idx / EPSILON_STEPS)

            # 各ステップでの報酬を計算。最終的にenv.stepを呼んでいる。
            new_rewards = exp_source.pop_rewards_steps()
            if new_rewards:
                # 学習が終わるかを判定、教科書では戻り値を判定してbreakしてある。boolなのですべき。
                reward_tracker.reward(new_rewards[0], step_idx, selector.epsilon)

            # replay bufferが貯まるまで待機
            ...
            optimizer.zero_grad() # グラディエントの初期化
            batch = buffer.sample(BATCH_SIZE)  # Replay Bufferからサンプル抽出
            loss_v = common.calc_loss(batch, net, tgt_net.target_model, GAMMA ** REWARD_STEPS, device=device) # 損失計算
            loss_v.backward() # 勾配を計算
            optimizer.step() # パラメーターの更新

            
            if step_idx % TARGET_NET_SYNC == 0:
                tgt_net.sync() # torch.nn.load_state_dictを呼ぶ。

            # 10000000回に一度、チェックポイントを作りモデルを保存
